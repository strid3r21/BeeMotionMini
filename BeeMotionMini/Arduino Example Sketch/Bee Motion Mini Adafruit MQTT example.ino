...
#define AIO_KEY      "your-adafruit-password"
...
void setup() {
 ...
 gpio_config(&config);
  esp_deep_sleep_enable_gpio_wakeup(BIT(DEFAULT_WAKEUP_PIN), DEFAULT_WAKEUP_LEVEL);  //setting the ESP to wake up from deep sleep when motion is detected.
  ...
  if (! test.publish("Motion Detected!")) { //put whatever you want to send to MQTT here. whether its a sensor reading or just a message like Motion Detected!
    Serial.println(F("Failed"));
  } else {
    Serial.println(F("OK!"));
  }
  digitalWrite(2, HIGH);   //turn on onboard LED
  ...
  esp_deep_sleep_start(); //going into deep sleep
